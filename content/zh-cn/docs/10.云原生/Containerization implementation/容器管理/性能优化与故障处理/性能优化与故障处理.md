...
[好用的镜像-有特殊功能](/docs/10.云原生/Containerization%20implementation/容器管理/好用的镜像-有特殊功能.md)

# 没有 bash 或 sh 等终端的容器排查

使用 busybox 检查网络情况

```bash
export ContainerName="loki"
docker run --name test_tools --rm -it --network container:${ContainerName} busybox
# 或者直接执行命令查看结果。这里执行的是 `nslookup pause` 命令检查容器内部 DNS 是否运行正常
docker run --name test_tools --rm -it --network container:${ContainerName} busybox nslookup pause
```

> [!Tip] 除了 busybox，也可以使用 nicolaka/netshoot 容器
