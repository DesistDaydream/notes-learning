...
> [!Tip] 在 Inventory 主文件中保存所有的变量并不是最佳的方式
> 我们通常在**独立的文件**中定义这些变量，这些独立文件与 inventory 文件保持关联. 不同于 inventory 文件(INI 格式)，这些独立文件的格式为 YAML。
>
> Ansible 使用 [host_group_vars](https://docs.ansible.com/projects/ansible/latest/collections/ansible/builtin/host_group_vars_vars.html) 插件实现在独立文件中设置的 主机变量 与 组变量。
...
>
> 文件的扩展名可以是： `.yml`, `.yaml`, `.json`, 无扩展名。
>
> 还可以把文件换成目录，目录以 组/主机 命名的文件（无扩展名），Ansible 会按字典顺序读取这些目录中的所有文件。
...
‘raleigh’ 组下的所有主机，都可以使用 ‘raleigh’ 组的变量。当变量变得太多时，分文件定义变量更方便我们进行管理和组织。还有一个方式也可参考，详见 Ansible Vault 关于组变量的部分。
