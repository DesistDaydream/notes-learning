...
> 参考：
>
> - [官方文档，用户指南 - 传统目录 - 循环](https://docs.ansible.com/ansible/latest/user_guide/playbooks_loops.html)
...
> [!Note]
> 在 Ansible 2.5 中添加了 `loop` ，作为一种更简单的循环方式，**建议在大多数场景中使用 `loop`**，而不是使用老旧的 `with_<lookup>`（暂时未弃用）。
>
> `loop` 和 `with_<lookup>` 是互斥的。虽然可以将它们嵌套在 `until` 下，但这会影响每次循环迭代。

## 循环的简单样例

下面展示了循环的基本功能：通过 `loop` 或者 `with_*` 来对一个列表中的值逐一操作
...
- `{{ item }}` 变量来引用 `loop` 关键字定义的内容
- `loop` 关键字来定义列表内容

> [!Attention] 在定义任何变量时，一定要避免使用 item 作为变量名
