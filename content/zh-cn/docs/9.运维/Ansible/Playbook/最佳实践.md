...
roles/
    common/               # 名为 common 的角色
        ...
        vars/             #
            main.yml      #  <-- 该角色的相关变量，优先级高一点（高于 组变量 与 主机变量）
        defaults/         #
            main.yml      #  <-- 该角色的默认变量，优先级较低（非常低，低于 组变量 与 主机变量）
        meta/             #
...
```bash
inventories/       # 这里存放 Inventory 目录，通过在命令行中使用 -i 选项以指定 Inventory 文件。目录名随意定。
   production/
...
一、指定多个 inventory（多个 inventory 主要是用来区分各种组的，以便人类更好理解分组逻辑；同时可以让后指定的 inventory 中的变量覆盖掉前面的相同名称的变量），并使用 --limit 限定主机

```bash
ansible-playbook -i inventory/a_common/ -i inventory/tian_jin/ deploy-monitoring.yaml --limit FJ-BS101-JMR-Monitor
```
