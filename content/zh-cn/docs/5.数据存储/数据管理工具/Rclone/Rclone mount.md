...
- **--dir-cache-time**(Duration) # 缓存目录条目的间隔时间。`默认值: 5m0s`。
    - i.e. rclone 每隔 5 分钟缓存一次后端信息，目的是：如果后端手动创建或删除了目录，挂在的磁盘里是看不到的，所以需要同步。
- **--poll-interval**(Duration) # 每次缓存后端目录条目后，等待的时间。仅在部分 Remote 上有效，设置的值必须小于 --dir-cache-time 标志。`默认值: 1m0s`。设置为 0 将会禁用等待时间

> 所以，每次后端更改了目录后， VFS 默认情况下，一共要等待 6 分钟才会感知到变化，如果着急，最好通过 [手动同步缓存](#手动同步缓存) 的方式立刻同步。不过  WebDAV 不支持 --poll-interval 标志，所以 WebDAV 等待 5 分钟。
...
对于 Windows 来说，无法使用官方文档提供的 `kill -SIGHUP $(pidof rclone)` 方式，所以：
