...
在 [Machine learning](/docs/12.AI/Machine%20learning/Machine%20learning.md) 领域，**Model(模型)** 可以代指多个不同层次的概念，可以是 数学公式、一个模型文件、一组模型文件、提供推理的模型服务、等等。
...
- **Parameters(参数)** # i.e. **Weight(权重)**，用于表示模型从数据中学习到的**特征**，也可以理解为模型的**状态**（特征/状态是一系列数值，用 **Tensor(张量)** 记录）。
  ...
  - 由于权重文件需要消耗大量空间，所以诞生多种 [权重格式](#权重格式)。
- **[Data processing](/docs/12.AI/Machine%20learning/Data%20processing.md)(数据处理)** # 提供 “人类可理解的信息” 与 “Tensor” 之间的相互转换逻辑。
...
> [!Example]
>
> 用 [Hugging Face](/docs/12.AI/Hugging%20Face.md) 生态的模型举例，一个完整的模型通常有[这些文件](/docs/12.AI/Machine%20learning/Transformer/Transformer.md#NLP%20关联文件示例)。
>
> 为了减少复杂性，也有很多设计将这些文件合并成一个文件，e.g. llama.cpp 的 GGUF 格式文件、etc.

模型需要 **[Training](/docs/12.AI/Machine%20learning/Training.md)(训练)**，训练后可以得到一系列的权重值（海量的 Tensor），这些 Tensor 以特定格式保存到文件中。Tensor 中通常是大量(上亿)个浮点数，如果进行了**模型量化**，也可以是整数。

训练好的模型可以进行 **[Inference](/docs/12.AI/Machine%20learning/Inference.md)(推理)**，依据输入通过计算预测输出。
...
- RNN
- GAN
- [Transformer](/docs/12.AI/Machine%20learning/Transformer/Transformer.md)
