...
**Machine learning(机器学习，简称 ML)** 是 [AI](/docs/12.AI/12.AI.md) 领域的一个研究方向。ML 致力于研究出一种**统计算法**，能够从数据中学习，并对未见过的数据进行 **generalization(泛化)**，从而**无需**明确的**编程指令**即**可执行任务**。
...
> [!Example]
> 自然语言处理中，当我们向模型提交输入的文字后，模型会根据参数，逐步预测将要输出的每一个文字，每个文字都是由模型根据训练好的参数，依据统计学的百分比概率推导出来的（选择可能性概率最高的那个字）。
...
根据 [Regression](obsidian://open?vault=notes-science&file=Math/Regression)(回归) 的学习。得知：回归是 **”倒推“**。那么根据一组数据”倒推“出其中的关系，这些关系也就是模型。那堆离散的点就像训练时的数据集，那条直线就是这组数据的模型。
