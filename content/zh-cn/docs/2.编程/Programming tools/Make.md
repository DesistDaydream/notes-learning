...
> - [手册](https://www.gnu.org/software/make/manual/html_node/index.html)
...
> - [GitHub，Makefile 文件教程](https://gist.github.com/isaacs/62a2d1825d04437c6f08)

GNU Make 是一个工具，它控制从程序的源文件生成程序的可执行文件和其他非源文件。

Make 通过名为 `Makefile` 的文件来获取构建程序所需的信息。Makefile 列出了所有非源文件以及如何根据其他文件计算这些文件。编写程序时，您应该为其编写一个 Makefile，以便可以使用 Make 来构建和安装程序。
...
**Recipe(配方)** # 定义实现目标要执行的动作。一个 Recipe 可以包含一个或多个命令

- <font color="#ff0000">注意: 每行 Rercipe 必须以 `\t`(制表符) 开头</font>

> [!Attention] 如果某行 Recipe 不以制表符开始，将会报错: `Makefile:<错发出现的行号>: *** missing separator.  Stop.`。只能是 `\t`(制表符)，4 个或者 2 个空白字符都不行

Target 是必需的；Prerequisites 和 Commands 都是可选的，但是两者之中必须至少存在一个。

每条规则就明确两件事：构建目标的前置条件是什么，以及如何构建。下面就详细讲解，每条规则的这三个组成部分。

### 2.2 Target(目标)

一个 Target(目标) 就构成一条规则。目标通常是文件名，指明 Make 命令所要构建的对象，比如上文的 a.txt 。Target 也可以是多个文件名，之间用空格分隔。

除了文件名，目标还可以是某个操作的名字，这称为 **Phony target(伪目标)**。
...
Recipe(配方) 表示如何更新目标文件，由一行或多行的 Shell 命令组成。它是构建"目标"的具体指令，它的运行结果通常就是生成目标文件。
