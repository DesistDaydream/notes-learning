...
- **FieldName(字段名称)** # 属性的名称
- **BaseType(基础类型)** # 属性的类型。Tips: 基础类型可以是另一个结构体，表示该结构体包含另一个结构体

每个字段都有其对应的基础数据类型，在一个结构体中，FieldName 必须是唯一的。
...
在 Go 语言中，这个 `.`(点) 符号叫做 **Selector(选择器)**。无论定义的变量是一个结构体类型还是一个结构体类型指针，都是用同样的 **selector-notation(选择器符)** 来引用结构体的字段。
