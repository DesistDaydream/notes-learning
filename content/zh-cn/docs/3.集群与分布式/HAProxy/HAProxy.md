...
- frontend 和 backend 主要用来实现 7 层代理。两个配置段相互配置。用来匹配代理规则的，用户访问哪一类页面，就把这类请求转发到某后端服务器上。可以与 [Nginx](/docs/Web/Nginx/Nginx.md) 的配置互相参考，acl 就是 naginx 的 location 里的匹配规则；backend 里的 server 就是 nginx 里 upstream 的 server；use_backend 就是 nginx 里的 proxy_pass。
