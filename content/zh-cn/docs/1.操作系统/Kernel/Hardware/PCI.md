...
**PCI device resouorces(PCI 设备的资源)** 数据以文件形式保存，*PCI 设备资源目录* 通常包含如下文件
...
**`/sys/devices/pci${DOMAIN:BUS}/${DOMAIN:BUS:SLOT.FUNC}/`** # [PCI 设备资源储存目录](#PCI%20设备资源目录)。包含该 PCI 设备的各种信息，e.g. 供应商、PCI 类型、etc.

- **/sys/bus/pci/devices/** # 可以在这里找到所有 PCI 设备的列表，该目录下是 PCI Addr 命名的软链接，指向在 `/sys/devices/pci${DOMAIN:BUS}/` 目录下的某个具体子目录。
...
该文件由 [pciutils](https://github.com/pciutils) 维护。在官方网站中有在线查询库可以在线查询，主要分为两大块查询
