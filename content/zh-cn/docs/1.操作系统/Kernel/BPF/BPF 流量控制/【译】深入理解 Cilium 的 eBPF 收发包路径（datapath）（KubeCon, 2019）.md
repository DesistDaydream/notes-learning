...
source: "https://arthurchiao.art/blog/understanding-ebpf-datapath-in-cilium-zh/"
author:
published:
created: 2025-12-25
description:
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/ebpf-sample.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/why-ebpf.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-l1-l2.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-driver-poll.png)

首先，NAPI poll 机制不断调用驱动实现的 poll 方法，后者处理 RX 队列内的包，并最终 将包送到正确的程序。这就到了我们前面的 XDP 类型程序。

### Step 2：XDP 程序处理

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-xdp.png)
...
### Step 3：clean\_rx()：创建 skb

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-clean-rx.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-gro.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-receive-skb.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-gxdp.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-tap.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-tc.png)
...
> 译者注：如何查看已加载的 eBPF 程序，可参考 [Cilium Network Topology and Traffic Path on AWS](https://arthurchiao.art/blog/cilium-network-topology-on-aws/) 。

### Step 9：Netfilter 处理

如果 tc BPF 返回 OK，包会再次进入 Netfilter。

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-netfilter.png)
...
### Step 10：L3 协议层处理：ip\_rcv()

最后，如果包没有被前面丢弃，就会通过网络设备的 `ip_rcv()` 方法进入协议栈的三层（ L3）—— 即 IP 层 —— 进行处理。

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-ip-rcv.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-netfilter-l4.png)

`ip_rcv()` 做的第一件事情是再次执行 Netfilter 过滤，因为我们现在是从四层（L4）的 视角来处理 socker buffer。因此，这里会执行 Netfilter 中的任何四层规则（L4 rules ）。

### Step 12：ip\_rcv\_finish() 处理

Netfilter 执行完成后，调用回调函数 `ip_rcv_finish()` 。

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-ip-rcv-finish.png)

`ip_rcv_finish()` 立即调用 `ip_routing()` 对包进行路由判断。

### Step 13：ip\_routing() 处理

`ip_routing()` 对包进行路由判断，例如看它是否是在 lookback 设备上，是否能 路由出去（could egress），或者能否被路由，能否被 unmangle 到其他设备等等。

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-ip-routing.png)

在 Cilium 中，如果没有使用隧道模式（tunneling），那就会用到这里的路由功能。相比 隧道模式，路由模式会的 datapath 路径更短，因此性能更高。

### Step 14：目的是本机：ip\_local\_deliver() 处理

根据路由判断的结果， **如果包的目的端是本机** ，会调用 `ip_local_deliver()` 方法。

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-ip-local-deliver.png)

`ip_local_deliver()` 会调用 `xfrm4_policy()` 。

### Step 15：xfrm4\_policy() 处理

`xfrm4_policy()` 完成对包的 **封装、解封装、加解密** 等工作。例如，IPSec 就是在这里完成的。

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-xfrm4-policy.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-udp-rcv.png)

`udp_rcv()` 对包的合法性进行验证，检查 UDP 校验和。然后，再次将包送到 `xfrm4_policy()` 进行处理。

### Step 17：xfrm4\_policy() 再次处理

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-xfrm4-policy-after-udp-rcv.png)

这里再次对包执行 transform policies 是因为，某些规则能指定具体的四层协议，所以只 有到了协议层之后才能执行这些策略。

### Step 18：将包放入 socket\_receive\_queue

这一步会拿端口（port）查找相应的 socket，然后将 skb 放到一个名为 `socket_receive_queue` 的链表。

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-socket-receive-queue.png)

### Step 19：通知 socket 收数据：sk\_data\_ready()

最后， `udp_rcv()` 调用 `sk_data_ready()` 方法，标记这个 socket 有数据待收。

![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/dp-highlight-sk-data-ready.png)
...
![](https://arthurchiao.art/assets/img/ebpf-datapath-in-cilium/userspace-sample.png)
...
| --- | --- | --- |
| Endpoint (includes Pods) | Endpoint | tc, cgroup socket BPF, sock\_ops BPF, XDP |
...
[« Cilium: What the Agents Do When ClusterMesh Enabled](https://arthurchiao.art/blog/cilium-code-clustermesh/) [\[译\] 大规模微服务利器：eBPF + Kubernetes（KubeCon, 2020） »](https://arthurchiao.art/blog/ebpf-and-k8s-zh/)