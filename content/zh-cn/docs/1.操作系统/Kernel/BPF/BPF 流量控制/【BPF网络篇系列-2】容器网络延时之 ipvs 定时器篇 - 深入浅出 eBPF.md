...
source: "https://www.ebpf.top/post/ebpf_network_kpath_ipvs/"
author:
  - "[[DavidDi]]"
...
- 操作系统为 CentOS 7.7.1908，内核版本为 3.10.0-1062.9.1.el7.x86\_64；

## 2\. 网络抖动问题

在容器集群中新部署的服务 A，在测试初期发现通过服务注册发现访问下游服务 B（在同一个容器集群） 调用延时 999 线偶发抖动，测试 QPS 比较小，从业务监控上看起来比较明显，最大的延时可以达到 200 ms。

![服务延时高](https://www.ebpf.top/post/ebpf_network_kpath_ipvs/imgs/service_latency_high.png)
...
!\[ping\_host\_container\] (imgs/ping\_host\_container.png)

图 2-2 ping 测试涉及到的主机和容器拓扑图

基于主机B 网络 eth1 和容器网络 cali-xxx 进行 ping 的对比结果如图：

![ping_host_container_detail](https://www.ebpf.top/post/ebpf_network_kpath_ipvs/imgs/ping_host_container_detail.png)
...
![ping_server_pcap.png](https://www.ebpf.top/post/ebpf_network_kpath_ipvs/imgs/ping_server_pcap.png)
...
优先级自上而下，HI\_SOFTIRQ 的优先级最高。其中 `NET_TX_SOFTIRQ` 对应于网络数据包的发送， `NET_RX_SOFTIRQ` 对应于网络数据包接受，两者共同完成网络数据包的发送和接收。
...
![netcard_dev_softirq](https://www.ebpf.top/post/ebpf_network_kpath_ipvs/imgs/netcard_dev_softirq.png)
...
![perf_kernel_cpu0.png](https://www.ebpf.top/post/ebpf_network_kpath_ipvs/imgs/perf_kernel_cpu0.png)
...
![estimation_timer_flamgraph](https://www.ebpf.top/post/ebpf_network_kpath_ipvs/imgs/estimation_timer_flamgraph.png)
...
![estimation_timer_funcgraph](https://www.ebpf.top/post/ebpf_network_kpath_ipvs/imgs/estimation_timer_funcgraph.png)
...
![timer_softirq_hist](https://www.ebpf.top/post/ebpf_network_kpath_ipvs/imgs/timer_softirq_hist.png)
...
### 3.3 ipvs estimation\_timer 定时器

谜底终将揭晓！

我们通过阅读 ipvs 相关的源码，发现 `estimation_timer()[ipvs]` 函数针对每个 Network Namespace 创建时候的通过 [ip\_vs\_core.c](https://elixir.bootlin.com/linux/v5.8/source/net/netfilter/ipvs/ip_vs_core.c#L2469) 中的 `__ip_vs_init` 初始化的，
...
`ip_vs_estimator_net_init` 函数在文件 [ip\_vs\_est.c](https://elixir.bootlin.com/linux/v5.8/source/net/netfilter/ipvs/ip_vs_est.c#L187) 中，定义如下：
...
`estimation_timer` 也定义在 [ip\_vs\_est.c](https://elixir.bootlin.com/linux/v5.8/source/net/netfilter/ipvs/ip_vs_est.c#L96) 文件中。
...
代码 3-7 ipvs estimation\_timer 函数

从 `estimation_timer` 的函数实现来看，会首先调用 spin\_lock 进行锁的操作，然后遍历当前 Network Namespace 下的全部 ipvs 规则。由于我们集群的某些历史原因导致生产集群中的 Service 比较多，因此导致一次遍历的时候会占用比较长的时间。
...
- **原文链接：** [https://www.ebpf.top/post/ebpf\_network\_kpath\_ipvs/](https://www.ebpf.top/post/ebpf_network_kpath_ipvs/)
...
- [kpatch 热修 ipvs estimation\_timer 内核延时问题](https://www.ebpf.top/post/kpatch_ipvs_timer/)
