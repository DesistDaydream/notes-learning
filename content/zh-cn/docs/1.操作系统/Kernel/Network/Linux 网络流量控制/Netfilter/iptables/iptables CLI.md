...
**recent 模块** # 动态创建一个 IP 地址列表，然后以几种不同的方式与该列表进行匹配。

- 例如，可以动态得创建一个“坏蛋”列表，列出那些试图连接到 22 端口的人，然后直接丢弃他们未来的所有数据包，而无需进一步考虑。
- **--name** # 列表名称
- **--set** # 创建列表
- **--rcheck** # 检查数据包的源地址当前是否在列表中
- **--seconds**(INT) # 此选项必须与 --rcheck 或 --update 其中之一结合使用。使用时，这将缩小匹配范围，仅当地址在列表中且在过去给定的秒数内被看到时才会匹配。
- **--hitcount**(INT) # 此选项必须与 --rcheck 或 --update 其中之一结合使用。使用时，这将缩小匹配范围，仅当地址在列表中且接收到的数据包数量大于或等于给定值时才会匹配。此选项可与 --seconds 一起使用，以创建更严格的匹配条件，要求在特定时间范围内达到一定的命中次数。
