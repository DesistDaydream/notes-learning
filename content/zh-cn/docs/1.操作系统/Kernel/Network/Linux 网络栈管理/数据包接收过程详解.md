...
>         1. [Linux 网络栈原理、监控与调优：前言](https://arthurchiao.art/blog/linux-net-stack-zh/)
>         2. [Linux 中断（IRQ/softirq）基础：原理及内核实现](https://arthurchiao.art/blog/linux-irq-softirq-zh/)
>         3. [Linux 网络栈接收数据（RX）：原理及内核实现](https://arthurchiao.art/blog/linux-net-stack-implementation-rx-zh/)
>         4. [Monitoring Linux Network Stack](https://arthurchiao.art/blog/monitoring-network-stack/)
>         5. [Linux 网络栈接收数据（RX）：配置调优](https://arthurchiao.art/blog/linux-net-stack-tuning-rx-zh/)

# Linux 网络栈接收数据（RX）：原理及内核实现（2022）

原文: https://arthurchiao.art/blog/linux-net-stack-implementation-rx-zh/

![800](https://arthurchiao.art/assets/img/linux-net-stack/rx-overview.png)
...
2. [Linux 中断（IRQ/softirq）基础：原理及内核实现](https://arthurchiao.art/blog/linux-irq-softirq-zh/)
3. [Linux 网络栈接收数据（RX）：原理及内核实现](https://arthurchiao.art/blog/linux-net-stack-implementation-rx-zh/)
4. Linux 网络栈发送数据（TX）：原理及内核实现（TBD）

**监控**

1. [Monitoring Linux Network Stack](https://arthurchiao.art/blog/monitoring-network-stack/)

**调优**

1. [Linux 网络栈接收数据（RX）：配置调优](https://arthurchiao.art/blog/linux-net-stack-tuning-rx-zh/)
2. Linux 网络栈发送数据（TX）：配置调优（TBD）
...
![](https://arthurchiao.art/assets/img/linux-net-stack/rx-overview.png)
...
![](https://arthurchiao.art/assets/img/linux-net-stack/l2-protocols.png)
...
![](https://arthurchiao.art/assets/img/gpu-notes/8x-a100-node-hw-topo.png)
...
![](https://arthurchiao.art/assets/img/linux-net-stack/pci-probe.png)
...
![](https://arthurchiao.art/assets/img/linux-net-stack/mlx5e_init.png)
...
![](https://arthurchiao.art/assets/img/linux-net-stack/dma-ringbuffer.png)
...
![](https://arthurchiao.art/assets/img/linux-net-stack/irq-and-napi-poll.png)
...
最后，更新一个硬件寄存器，记录硬件中断频率。 可以用 ethtool **调整 IRQ 触发频率** 。
...
![](https://arthurchiao.art/assets/img/linux-net-stack/cpu-schedule-threads.png)
...
![](https://arthurchiao.art/assets/img/linux-net-stack/run_ksoftirqd.png)
...
![](https://arthurchiao.art/assets/img/linux-net-stack/uu_do_softirq.png)
...
![](https://arthurchiao.art/assets/img/linux-net-stack/net_rx_action.png)
...
另外， `time_squeeze` 升高 **==并不一定表示系统有丢包==** ，只是表示 softirq 的收包预算用完时，RX queue 中仍然有包等待处理。只要 RX queue 在下次 softirq 处理之前没有溢出，那就不会因为 time_squeeze 而导致丢包；但如果有持续且大量的 time_squeeze，那确实有 RX queue 溢出导致丢包的可能，需要结合其他监控指标 来看。在这种情况下，调大 budget 参数是更合理的选择：与其让网卡频繁触发 IRQ->SoftIRQ 来收包，不如让 SoftIRQ 一次多执行一会，处理掉 RX queue 中尽量多的 包再返回，因为中断和线程切换开销也是很大的。
...
![](https://arthurchiao.art/assets/img/linux-net-stack/napi_gro_receive.png)

Fig. GRO receive process: heavy-lifting work before entering kernel stack

`napi_gro_receive()` 负责处理数据，并将数据送到协议栈，大部分相关的逻辑在函数 `dev_gro_receive()` 里实现：
...
然后遍历一个 offload filter 列表，如果高层协议（L3/L4）认为其中一些数据属于 GRO 处理的范围，就会允许其对数据进行操作。 协议层的返回是 `struct sk_buff *` 指针，指针非空表示这个 packet 需要做 GRO，非空表示可以送到协议栈 。另外，也可以通过这种方式传递一些协议相关的信息。例如，TCP 协议需要判断是否以及合适应该将一个 ACK 包合并到其他包里。
...
![](https://arthurchiao.art/assets/img/linux-net-stack/netif_receive_skb_list_internal.png)
...
检查是否插入了 tap（探测点），这些 tap 是抓包用的。例如， `AF_PACKET` 地址族就可以插入这些抓包指令，一般通过 `libpcap` 库。 如果存在 tap，数据就会送到 tap，然后才到协议层。
...
RPS 在不同 CPU 之间分发 packet，但是，如果一个 flow 特别大，会出现单个 CPU 被打爆，而 其他 CPU 无事可做（饥饿）的状态。因此引入了 flow limit 特性，放到一个 backlog 队列的属于同一个 flow 的包的数量不能超过一个阈值。这可以保证即使有一个很大的 flow 在大量收包 ，小 flow 也能得到及时的处理。
...
![](https://arthurchiao.art/assets/img/linux-net-stack/inet_init.png)
...
![](https://arthurchiao.art/assets/img/linux-net-stack/l3-processing-stack.png)
...
这里做了一个优化。为了能将包送到合适的目的地，需要一个路由子系统的 `dst_entry` 变量。为了获取这个变量，早期的代码调用了 `early_demux` 函数。 `early_demux` 是一个优化项，通过检查相应的变量是否缓存在 `socket` 变量上，来路由这个包所需要的 `dst_entry` 变量。默认 `early_demux` 是打开的：
...
![](https://arthurchiao.art/assets/img/linux-net-stack/uu_udp4_lib_rcv.png)
...
前面提到，网络栈可以收集包的时间戳信息。如果使用了 RPS 功能，有相应的 `sysctl` 参数可以控制何时以及如何收集时间戳；一些网卡甚至支持在硬件上打时间戳。 如果想看 **==内核网络栈给收包增加了多少延迟==** ，那这个特性非常有用。
...
从上面这个信息看，该网卡不支持硬件打时间戳。但这个系统上的软件打时间戳，仍然可以帮助判断内核在接收路径上到底带来多少延迟。
...
对单个 socket 设置此选项，需要传一个以微秒（microsecond）为单位的时间，内核会在这个时间内对设备驱动的接收队列做 busy poll。当在这个 socket 上触发一个阻塞式读请求时，内核会 busy poll 来收数据。
...
Linux 网络栈很复杂。 对于这样复杂的系统（以及类似的其他系统），如果不能在更深的源码层次上理解它正在做什么，就无法做好监控和调优；而有了对内核网络栈的完整理解，就会发现一片新天地， 对于一些哪怕是“常规问题”的解决，也会有不一样的认识。

# 参考资料

1. [Linux 网络栈监控和调优：接收数据（2016）](https://arthurchiao.art/blog/linux-net-stack-implementation-rx-zh/%7Blink%20_posts/2018-12-05-tuning-stack-rx-zh.md%7D)
2. [Monitoring Linux Network Stack](https://arthurchiao.art/blog/monitoring-network-stack/)
3. [(译) 深入理解 Cilium 的 eBPF 收发包路径（datapath）](https://arthurchiao.art/blog/understanding-ebpf-datapath-in-cilium-zh/)
4. [XDP (eXpress Data Path)：在操作系统内核中实现快速、可编程包处理（ACM，2018）](https://arthurchiao.art/blog/xdp-paper-acm-2018-zh/)

[« Linux 中断（IRQ/softirq）基础：原理及内核实现（2022）](https://arthurchiao.art/blog/linux-irq-softirq-zh/) [Linux 网络栈接收数据（RX）：配置调优（2022） »](https://arthurchiao.art/blog/linux-net-stack-tuning-rx-zh/)
