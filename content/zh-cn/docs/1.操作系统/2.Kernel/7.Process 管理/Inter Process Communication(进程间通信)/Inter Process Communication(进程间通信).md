...
- **Socket(套接字)** # 计算机领域中数据通信的一种约定，或者说是一种方法，《[Socket(套接字)](/docs/1.操作系统/2.Kernel/7.Process%20管理/Inter%20Process%20Communication(进程间通信)/Socket(套接字)/Socket(套接字).md)》。Socket 又分为两种
  - Unix Domain Socket
  - Network Socket
- **Shared Memory(共享内存)** #
