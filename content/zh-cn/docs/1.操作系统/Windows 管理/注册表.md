...
> 参考：
>
> - [Wiki, Windows Registry](https://en.wikipedia.org/wiki/Windows_Registry)
...
![](https://notes-learning.oss-cn-beijing.aliyuncs.com/windows/registry/regedit_index_1.png)
...
Key 是以 `\` 分割的层次结构，有 5 个预定的 Root keys(根键/根项)

- **HKEY_CLASSES_ROOT(或 HKCR)** # 有关已注册应用程序的信息，包括 驱动、文件拓展名、etc.
- **HKEY_CURRENT_USER(或 HKCU)** # 特定于当前登录用户的设置，包括 环境变量、个人桌面的设置、etc.
- **HKEY_LOCAL_MACHINE(或 HKLM)** # 特定于本地计算机的设置，由系统内核维护在内存中 ， 以便映射所有其他子键 。 这些信息可以其它用户使用。
- **HKEY_USERS(或 HKU)** # 和上面的第二个项目相对，包含与机器上主动加载的每个用户配置文件的 HKEY_CURRENT_USER 键对应的子键
- **HKEY_CURRENT_CONFIG(或 HKCC)** # 计算机当前硬件的配置信息，这些配置可以根据当前所连接的网络类型或硬件驱动软件安装的改变而改变。

> Notes: 还有 2 个 Root keys 仅存在与 WindowsNT 或 Windows9x，就不记录了
