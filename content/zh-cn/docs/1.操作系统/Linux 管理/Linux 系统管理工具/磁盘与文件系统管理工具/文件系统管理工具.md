...
转存 [EXT FileSystem](docs/1.操作系统/Kernel/Filesystem/磁盘文件系统/EXT%20FileSystem.md) 文件系统的信息。说白话就是显示指定设备上存在的各种 ext 格式的文件系统的信息（包括 超级块、块组、等 信息）。
...
resize 是调整 [EXT FileSystem](docs/1.操作系统/Kernel/Filesystem/磁盘文件系统/EXT%20FileSystem.md) 容量大小的工具

> [!Note] 如果是 xfs 文件系统，则使用 xfs_growfs 命令来增大或者收缩未加载的 ext 文件系统的大小。

> [!Attention] 如果文件系统是处于 mount 状态下，那么它只能做到扩容

> [!Tip] 在 lv 扩容完成后，还需要对文件系统进行扩容，否则无法使用
