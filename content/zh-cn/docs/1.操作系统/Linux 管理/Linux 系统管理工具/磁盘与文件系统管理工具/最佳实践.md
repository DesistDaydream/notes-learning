...
- 为 /dev/vdb1 创建 [XFS](/docs/1.操作系统/Kernel/Filesystem/磁盘文件系统/XFS.md) 文件系统
  - mkfs.xfs /dev/vdb1
    - 或者为 /dev/vdb1 创建 [EXT FileSystem](/docs/1.操作系统/Kernel/Filesystem/磁盘文件系统/EXT%20FileSystem.md)
        - mkfs.ext4 /dev/vdb1
- 挂载文件系统
...
MOUNTS=(/data1 /data2 /data3 /data4 /home)
...
for i in "${!DISKS[@]}"; do
  ...
  echo "UUID=${uuid}  ${mp}  ext4  defaults  0  2" | tee -a /etc/fstab
done

# 4. 挂载验证
mount -a
df -h | grep -E 'data[1-4]|/home'
