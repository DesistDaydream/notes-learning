...
> 参考：
>
> - [GitHub 项目，smartmontools/smartmontools](https://github.com/smartmontools/smartmontools)
> - [官网](https://www.smartmontools.org/)
> - [官方 Manual(手册), smartctl(8)](https://www.smartmontools.org/browser/src/smartctl.8.in)
...
> lsscsi 命令在 **只有 `type` 为 `disk` 时才表示为一块硬盘，其它如 `loop` 则应该过滤掉。** 每块硬盘中的 `children` 表示它下面的分区，通过 `mountpoint` 可确定硬盘在此系统上的使用情况。
