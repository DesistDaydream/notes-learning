...
ln 工具用来在文件之间建立 [文件管理](/docs/1.操作系统/Kernel/Filesystem/文件管理/文件管理.md) 章节中提到的 **[Symbolic link](/docs/1.操作系统/Kernel/Filesystem/文件管理/Symbolic%20link.md)(符合链接)**

## Syntax(语法)

**ln \[OPTIONS] SourceFILE DestinationFILE**

> [!Tip] Man 手册中 `ln [OPTION]... [-T] TARGET LINK_NAME` TARGET 指源文件，LINK_NAME 指要新建的符号链接类型的文件

OPTIONS

- **-s, --symbolic** # 使用软连接。默认为硬连接
- **-f** # 若 LINK_NAME 文件已存在，则删除后创建。

## EXAMPLE

创建一个名为 mgmt 的软链接文件，指向 /usr/local/bin/marmot_26.09.22build21_amd64 文件

- ln -s /usr/local/bin/marmot_26.09.22build21_amd64 mgmt
