...
-H，-L 和-P 选项控制 find 处理 [Symbolic link](/docs/1.操作系统/Kernel/Filesystem/文件管理/Symbolic%20link.md)(符号链接) 的行为。这些参数之后的命令行参数将被视为要检查的文件或目录的名称，直到以 `-` 开头的第一个参数或参数 `(` 或 `!` 。符号链接的概念详见：
