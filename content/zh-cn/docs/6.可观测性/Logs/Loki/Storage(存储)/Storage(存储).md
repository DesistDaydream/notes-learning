...
与 [Prometheus 的存储概念](/docs/6.可观测性/Metrics/Prometheus/Storage/Storage.md)类似，Loki 也是将日志流数据抽象为一个一个的 Block(块)，只不过，在 Loki 这里，称之为 [Table(表)](#Table(表)%20概念)。由于 Loki 需要存储 Index 与 Chunks 两种数据，所以，数据在磁盘上的布局，与 Prometheus 也就不太一样了。
