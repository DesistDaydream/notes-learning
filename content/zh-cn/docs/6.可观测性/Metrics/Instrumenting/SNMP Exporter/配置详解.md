...
modules:
  ModuleName:  # 模块的名字，可以随意写。写 fuck_you 都行
    ...
    lookups:  # 常用来将某个 OID 作为索引相同的其他指标的 Label
     ...
     overrides: # 可以Allows for per-module overrides of bits of MIBs
       metricName: # 定义要改变的指标名称
         ignore: true # Drops the metric from the output.
         regex_extracts: # 根据 regex 定义的表达式提取 SNMP walk 到的值，并将值改为 value 定义的内容
           Temp: # 该字段可以用 "" 设为空。若不为空，则将会创建一个新的指标，将该字段附加到要改变的指标名称后面，比如这里的 Temp 将会创建一个名为 metricNameTemp 的指标
