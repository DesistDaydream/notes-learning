...
|        | 产品线  | 系列  | 代际      | 型号数字  | 后缀  |
| ------ | ---- | --- | ------- | ----- | --- |
| **描述** | 字符串  | 字符串 | 1-2 位数字 | 3 位数字 | 字符串 |
| **示例** | Core | i5  | 14      | 600   | KF  |

- **系列**：i3 < i5 < i7 < i9，代表定位档次（入门→旗舰），不代表代数
- **代际**：型号数字里的前 1-2 位，如 `14600` 中的 `14` = 第14代（Raptor Lake Refresh）
- **型号数字**：代际去掉后剩下的3位数字（如 `600`），数字越大通常性能越强，但**不是Nvidia那种固定的"50/60/70/80/90"对应关系**，同一代里300、400、500、600、700、900这类数字大致对应低→高端，但具体划分并非统一标准，需查 ARK 确认
- **常见后缀**：
    - **K**：解锁倍频，支持超频，**带核显**
    - **KF**：解锁倍频，支持超频，**无核显**
    - **F**：锁频，不支持超频，**无核显**
    - 无后缀：锁频，**带核显**（标准版）
    - **T**：超低功耗版（少见，多用于OEM/工控）
