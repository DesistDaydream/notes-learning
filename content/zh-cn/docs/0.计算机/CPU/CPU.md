...
- 不再用 "i3/i5/i7"，改为 **Core Ultra 5 / 7 / 9**
- 用 **"Series 1"（100系列，Meteor Lake）/ "Series 2"（200系列，Arrow Lake/Lunar Lake）** 区分世代，而非直接的代数数字
- 型号数字同样是 3 位，越大定位越高，但不同后缀（H/U/V/HX/K）分别对应不同产品线，数字不能跨后缀直接比较
- 后缀含义大致延续：**K/KF**（桌面超频）、**H/HX**（移动高性能）、**U/V**（移动低功耗，V 专用于 Lunar Lake 超低功耗轻薄本）
