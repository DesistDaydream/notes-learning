...
当我们输入完密码，经过 [Access Control(访问控制)](docs/IT学习笔记/1.操作系统/5.登录%20Linux%20与%20访问控制/Access%20Control(访问控制)/Access%20Control(访问控制).md) 相关程序的认证之后，login 工具会为我们分配一个 ttyX 的终端设备，然后我们就可以通过 tty 所关联的 Shell(通常是 bash)，与系统进行交互
...
详见：[Secure Shell(SSH) 安全外壳协议](/docs/IT学习笔记/1.操作系统/5.登录%20Linux%20与%20访问控制/Secure%20Shell(SSH)%20安全外壳协议/Secure%20Shell(SSH)%20安全外壳协议.md)
...
详见 [Account Manager(账户管理)](/docs/IT学习笔记/1.操作系统/5.登录%20Linux%20与%20访问控制/Account%20Manager(账户管理)/Account%20Manager(账户管理).md)
