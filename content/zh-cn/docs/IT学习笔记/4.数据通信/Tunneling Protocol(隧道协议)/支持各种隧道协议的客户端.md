...
- **yaml: <OBJECT> **# 通过 YAML 格式进行预处理
- **code: <STRING>** # 通过 JavaScript 代码进行预处理
...
- **prepend-rules: <\[]STRING>** # 数组 数组合并至原配置 rules 数组前
...
- **{ axios, yaml, notify, console }** # 导入的工具类对象/方法。可以直接在 `parse()` 方法中调用
  ...
  - console # 这里的 console 可以将内容输出至文件，方便调试。在 Settings 界面中 Profiles.Console Output 出可以打开该文件
    ...
    - 该文件默认保存在 `C:\Users\用户名\AppData\Local\Temp\cw-parser.log` 处
