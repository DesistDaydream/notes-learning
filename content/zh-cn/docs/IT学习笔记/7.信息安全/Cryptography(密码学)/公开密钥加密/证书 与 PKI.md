...
**Certificate(证书)** 在密码学中，是指 [公开密钥加密](.md) 中完善其签名[缺点](.md)的 **Public Key Certificate(公钥证书)**。在[公开密钥加密](.md)的介绍中，我们看到了公钥加密的特点，并且也发现了缺点，公钥容易被劫持。那么为了解决这个问题，就需要一个东西可以**验证公钥的真实性**。公钥证书也就由此而来。
