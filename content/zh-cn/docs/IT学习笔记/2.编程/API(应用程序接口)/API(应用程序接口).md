...
> 参考：[wiki](https://en.wikipedia.org/wiki/API)
...
API 的设计对其使用有重大影响。\[2]信息隐藏的原理将编程接口的作用描述为通过隐藏模块的实现细节来实现模块化编程，从而使模块用户无需了解模块内部的复杂性。\[32]因此，API 的设计试图仅提供用户期望的工具。\[2]编程接口的设计是软件体系结构的重要组成部分，是复杂软件的组织。\[33]
